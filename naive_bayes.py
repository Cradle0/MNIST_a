...
def train_mle_estimator(train_images, train_labels):
    ...
    # Find the total number of pixels at position j

    theta_mle = np.dot(train_images.T, train_labels)
    theta_mle = np.divide(theta_mle,np.shape(train_images)[0])


    pi_mle = np.divide(np.sum(train_labels,axis=0),np.shape(train_labels)[0])
...
def train_map_estimator(train_images, train_labels):
    ...
    # YOU NEED TO WRITE THIS PART
    theta_map = np.dot(train_images.T, train_labels)
    theta_map = np.divide(theta_map+2, np.shape(train_images)[0]+4)
    
    # Do a uniform thing a = 1 b = 1
    pi_map = np.divide(np.sum(train_labels,axis=0),np.shape(train_labels)[0])
...
def log_likelihood(images, theta, pi):
    ...
    log_like = np.dot(images,np.log(theta)) +  np.dot(1 - images,np.log(1 - theta)) + np.log(pi)
    # To get the denominator, take the exp of above. Then sum n log
    denom = scipy.special.logsumexp(log_like)

    return log_like - denom
...
def accuracy(log_like, labels):
    ...
    # YOU NEED TO WRITE THIS PART
    acc = np.count_nonzero(np.argmax(log_like,axis=1) ==  np.argmax(labels,axis=1)) / (np.shape(labels)[0])
