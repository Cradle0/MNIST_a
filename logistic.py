...
def logistic_predict(weights, data):
    ...
    y = sigmoid( (np.dot(data, weights[:len(weights) - 1]) ) + weights[len(weights) - 1] )
...
def evaluate(targets, y):
    ...
    ce = 1 / (len(targets)) * ( np.dot(-targets.T,np.log(y))  - np.dot( (1 - targets).T, np.log(1-y)) ) 

    # frac_correct = greater than half is t = 1 otherwise 0
    targets_hit = len( (np.where(y >= 0.5))[0] )
    frac_correct = targets_hit / len(targets)
...
def logistic(weights, data, targets, hyperparameters):
    ...
    df_nobias = 1 / len(y) * ( (data.T).dot(diff) )
    bias = 1 / len(y) * ( np.sum(diff) )


    df = (np.append(df_nobias, bias)).reshape(len(df_nobias) + 1, 1) 
    #####################################################################
    #                       END OF YOUR CODE                            #
    #####################################################################
    return f, df, y
