...
def run_knn():
    ...
    hyperparam = [1,3,5,7,9]
    rate = [0,0,0,0,0]

    for k in hyperparam:
        valid_labels = knn(k, train_inputs, train_targets, valid_inputs) 
        rate[int(np.floor(k/2))] = np.sum(valid_targets == valid_labels) / len(valid_targets)

    plt.plot(hyperparam, rate,'ro')
    ...
    for k in [3,5,7]:
        ...
        print(k, " = ", np.sum(test_targets == test_labels) / len(test_targets))
